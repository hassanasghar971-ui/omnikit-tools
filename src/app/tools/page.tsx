  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-10 sm:px-6">
      <div className="mb-8 flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-end">
        <div>
          <p className="mb-1 text-xs font-semibold uppercase tracking-[0.25em] text-cyan-400">Tool directory</p>
          <h1 className="text-3xl font-bold tracking-tight text-white">
            {q ? <>Results for “{q}”</> : category ? <>All {CATEGORY_MAP.get(category)?.name}</> : <>All 750 tools</>}
          </h1>
          <p className="mt-2 text-sm text-slate-400">
            {base.length} matching tools · {unlockedCount} live in this week&apos;s index window · every tool runs 100% on-device
          </p>
        </div>
