...
import { DAY_MS, hashString, slugify } from "@/lib/utils";
...
function buildSummary(name: string, category: CategorySlug): string {
  const cat = CATEGORY_MAP.get(category);
  return `${name} — instant, private, on-device ${cat?.name ?? category} tool. No uploads, no signup, no data retention, 100% free.`;
}
  ...
  for (const cat of CATEGORIES) {
    for (const name of TOOL_NAMES[cat.slug]) {
      ...
      tools.push({
        ...
        globalIndex: index++,
        summary: buildSummary(name, cat.slug),
...
/* ------------------------------------------------------------------ */
/* Controlled drip-feed unlock schedule (anti scaled-content abuse)    */
/* ------------------------------------------------------------------ */

/**
 * Anchor date for the drip-feed. 24 tools are indexable on day one, then
 * 16 more unlock every day until the full 750-tool registry is exposed to
 * crawlers — preventing scaled-content-abuse flags.
 * Update this constant on deployment day to restart the schedule fresh.
 */
export const TOOLS_EPOCH = Date.UTC(2026, 7, 1);

/** Tools live on day one… */
export const INITIAL_UNLOCKS = 24;
/** …and 16 more unlock every day until all 750 are indexed. */
export const DAILY_UNLOCKS = 16;

export function unlockedToolCount(now: number = Date.now()): number {
  const days = Math.max(0, Math.floor((now - TOOLS_EPOCH) / DAY_MS));
  return Math.min(ALL_TOOLS.length, INITIAL_UNLOCKS + days * DAILY_UNLOCKS);
}

export function isUnlocked(tool: ToolMeta, now: number = Date.now()): boolean {
  return tool.globalIndex < unlockedToolCount(now);
}

export function unlockedTools(now: number = Date.now()): ToolMeta[] {
  return ALL_TOOLS.filter((t) => isUnlocked(t, now));
}

export function isRecentlyUnlocked(tool: ToolMeta, now: number = Date.now()): boolean {
  const days = Math.floor((now - TOOLS_EPOCH) / DAY_MS);
  return (
    tool.globalIndex >= INITIAL_UNLOCKS + (days - 1) * DAILY_UNLOCKS &&
    isUnlocked(tool, now)
  );
}
